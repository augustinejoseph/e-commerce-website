...
def adminLogin(request):
    if request.method == "POST":
        ...
        if user is not None and user.is_superadmin:
            login(
                request, user
            )  # <-- use the login() function to authenticate the user
...
class AdminDashboard(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        sales_data = (
            OrderProduct.objects.filter(createdAt__range=(start_date, end_date))
            .annotate(day=TruncDay("createdAt"))
            .values("day")
            .annotate(total_sales=Sum("productPrice"))
        )
        print(end_date)
        print(start_date)
        print(sales_data)
...
def ProductSearch(request):
    if "keyword" in request.GET:
        ...
        products = Product.objects.order_by("createdDate").filter(
            Q(productDescription__icontains=keyword) | Q(productName__icontains=keyword)
        )
...
class ProductCreateView(CreateView):
    ...
    def form_invalid(self, form):
        ...
        messages.error(
            self.request, "There was an error creating the product. Please try again."
        )
        return response
...
def CategorySearch(request):
    if "keyword" in request.GET:
        ...
        categories = Category.objects.filter(
            Q(categoryName__icontains=keyword) | Q(slug__icontains=keyword)
        )
        print(categories)
...
def OrderSearch(request):
    if "keyword" in request.GET:
        keyword = request.GET.get("keyword")
        orders = Order.objects.order_by("createdDate").filter(
