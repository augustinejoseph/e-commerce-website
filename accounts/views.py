...
def activate(request, uidb64, token):
    try:
        # uid will give the primary key of the user after decoding
        uid = urlsafe_base64_decode(uidb64).decode()
        user = Account._default_manager.get(pk=uid)
    except (TypeError, ValueError,  OverflowError, Account.DoesNotExist):
        # if any of the above error occurs, the user is set to none
        user = None
...
def forgotPassword(request):
    if request.method == 'POST':
        email = request.POST['email']
        # checks if an account with that email exists in the databsse. .exists() returns True or False.
...
def resetpasswordValidate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        # save the uid inside session to use it later when resetting the password
...
def resetPassword(request):
    # we have the id inside the session now. So only request is needed.
    if request.method == 'POST':
        password = request.POST['password']
        ...
        if password == confirm_password:
            ...
            # hash the password and save it(set_password())
