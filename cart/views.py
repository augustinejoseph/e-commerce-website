            ...
            for item in request.POST:
                ...
                try:
                    variation = Variations.objects.get(
                        product=product, variationValue__iexact=value
                    )
                    product_variation.append(variation)
                except:
                    pass
        is_cartItem_exists = CartItem.objects.filter(
        ...
        if request.method == "POST":
            for item in request.POST:
                ...
                try:
                    variation = Variations.objects.get(
                        product=product,
                        variationCategory__iexact=key,
                        variationValue__iexact=value,
                    )
                    product_variation.append(variation)
                except:
                    pass
...
def removeCartItem(request, productId, cartItemId):
    ...
    if request.user.is_authenticated:
        cartItem = CartItem.objects.get(
            product=product, user=request.user, id=cartItemId
        )
    else:
        cart = Cart.objects.get(cartId=_cartId(request))
        ...
        try:
            ...
            if self.request.user.is_authenticated:
                cartItems = CartItem.objects.filter(
                    user=self.request.user, isActive=True
                )
            else:
            ...
            if coupon_id:
                ...
                discount_amount = utils.discount_amount(
                    total_with_tax, discount_percentage
                )
